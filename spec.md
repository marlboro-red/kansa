...
- `ui~grp-assign~1` — In the classifier and inventory views, `g` on a selected requirement (or multi-selection) shall open group quick-assign (fuzzy-find existing groups, create inline); assignment shall be one keystroke plus a name, not a form.
