...
- `ui~panel-resize~1` — The right-hand panel (inventory / reconciliation / proposals share the slot) shall be resizable by dragging its divider, between 260px and 760px and never wider than the window allows; double-clicking the divider resets it, and the width is persisted per machine.
...
- Reconciliation review: after an HLD edit, the verdict list (unchanged / reworded / meaning-changed / new / missing) presented for human confirmation *before* the round closes; `missing` items block closure until retired-with-reason or re-anchored. A verdict's `req~` chips open an inline card — statement, status, pattern, rating/owner, notes, suspect, and whether this sentence is the requirement's *only* anchor — so the decision is made against the requirement, not just its id.
